...
def parse_TYPE_DIGITAL():
	...
	print("index of Type Digital is " + str(index_of_file))#debug
	file = removeBracesNotInQuotes(file, index_of_file)
...
def parse_ROLE():
	...
	while file[index_of_file] != "]\n":
		#remove braces of first line
		file[index_of_file] = file[index_of_file].replace("[(", "['")
		file[index_of_file] = file[index_of_file].replace("),(", "','")
		file[index_of_file] = file[index_of_file].replace("),\n", "',\n")
		index_of_file += 1
		#preface quotes in second line
		file[index_of_file] = file[index_of_file].replace("'", "\\'")
		file[index_of_file] = file[index_of_file].replace('"', '\\"')
...
def parse_LANGUAGE():
	...
	#For Language:
	#	Preface all ' and " with \
	#	For each line:
	#		Replace 1st and 2nd (
	#		Replace 1st and last )
	#	Go back through and remove all () not in ''
...
def write_new_constants_py():
	...
	with open("new_constants.py", "w") as new_file:
		for line in file:
			new_file.write(line)
...
def removeBracesNotInQuotes(file, index_of_file):
	while file[index_of_file] != "]\n":
		file[index_of_file] = file[index_of_file].replace("('", "'")
		file[index_of_file] = file[index_of_file].replace("')", "'")
		file[index_of_file] = file[index_of_file].replace("(", "", 1)
