...
$(document).ready(function() {
  ...
  $("#jquery_jplayer_1").jPlayer({
    ...
    timeupdate: function(event) {
      lastIndex = transcriptUpdate(event,jsonobject,lastIndex);
    },
...
function transcriptUpdate(event,jsonobject,lastIndex) {
  var currentIndex = lastIndex;
  var currentTimeAsInt = event.jPlayer.status.currentTime;
  var lastTimeAsInt = timecodeToInt(jsonobject.text[lastIndex].timecode);
  
  var divId = '';
  var hexColor = '';
  
  if(currentTimeAsInt >= lastTimeAsInt) {
    
    if((lastIndex-1) >= 0) {
      
      divId = '#ts';
      divId += (lastIndex-1).toString();
      hexColor = rgb2hex($(divId).css('color'));
      
      if(hexColor === '#787788') { //if #ts[lastIndex-1] is highlighted
        highlight(divId,'off'); //animate it off
      }
    
    }
    
    divId = '#ts';
    divId += lastIndex.toString();
    hexColor = rgb2hex($(divId).css('color'));
    
    if(hexColor === '#f7f7f7') { //if #ts[lastIndex] is not highlighted
      highlight(divId,'on'); //animate it on
    }
    
    var nextTimeAsInt = timecodeToInt(jsonobject.text[lastIndex+1].timecode);
    
    if(currentTimeAsInt >= nextTimeAsInt) {
      currentIndex++;
    }
